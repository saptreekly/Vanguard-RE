//! Frozen Mandiant/pefile ImpHash ordinal → name tables.
//!
//! These must match pefile `ordlookup/imphash_*.py` / YARA `pe_utils.c` so
//! ImpHash values stay compatible with VirusTotal. Do not "improve" them.
...
fn lookup_table(table: &[(u16, &'static str)], ordinal: u16) -> Option<&'static str> {
    table
...
/// Resolve an ordinal for ImpHash using the frozen pefile tables.
/// `dll` should be the lowercased module name *with* extension (e.g. `ws2_32.dll`).
pub fn resolve_imphash_ordinal(dll: &str, ordinal: u16) -> String {
    let name = match dll {
        "ws2_32.dll" | "wsock32.dll" => lookup_table(WS2_32, ordinal),
        "oleaut32.dll" => lookup_table(OLEAUT32, ordinal),
        _ => None,
    };
    match name {
...
/// Parse goblin/pefile-style ordinal function labels (`ORDINAL 8`, `#8`, `ord8`).
pub fn parse_ordinal_label(func: &str) -> Option<u16> {
    let f = func.trim();
    let lower = f.to_ascii_lowercase();
    if let Some(rest) = lower.strip_prefix("ordinal ") {
        return rest.trim().parse().ok();
    }
    if let Some(rest) = f.strip_prefix('#') {
        return rest.trim().parse().ok();
    }
    if let Some(rest) = lower.strip_prefix("ord") {
...
mod tests {
    ...
    #[test]
    fn ws2_32_ordinal_8_is_htonl() {
        assert_eq!(resolve_imphash_ordinal("ws2_32.dll", 8), "htonl");
        assert_eq!(resolve_imphash_ordinal("wsock32.dll", 8), "htonl");
    }

    #[test]
    fn unknown_dll_falls_back_to_ordn() {
        assert_eq!(resolve_imphash_ordinal("kernel32.dll", 8), "ord8");
    }

    #[test]
    fn parses_ordinal_labels() {
        assert_eq!(parse_ordinal_label("ORDINAL 8"), Some(8));
        assert_eq!(parse_ordinal_label("Ordinal 115"), Some(115));
        assert_eq!(parse_ordinal_label("#8"), Some(8));
        assert_eq!(parse_ordinal_label("ord8"), Some(8));
        assert_eq!(parse_ordinal_label("CreateFileA"), None);
    }
}
